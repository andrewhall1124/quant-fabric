...
class Backtester:
    ...
    def __init__(self, start_date: date, end_date: date, strategy):
    ...
    def run(self):
        ...
        pnl = merged.group_by('date').agg(
            weighted_ret_mean=pl.col('weighted_ret').sum()
        ).sort(by=['date'])

        pnl = pnl.with_columns(
            pl.col('weighted_ret_mean').alias('portfolio_ret')
        ).with_columns(
